...
| `product/PRDs/{area}/reviews/` | by filename suffix: `{slug}-assumption-map.md` → `/assumption-map` · `{slug}-red-team.md` → `/red-team` · `{slug}-challenge-{YYYY-MM-DD}.md` → `/prd-challenge` (dated, one per run) · `{slug}-premortem.md` → `/pre-mortem` · `{initiative-slug}-{job-slug}-job-spec-review-{YYYY-MM-DD}.md` → `/job-spec-draft` challenge mode (dated, one per run) — the skills also run as `/prd-challenge` lenses and keep these same filenames |
