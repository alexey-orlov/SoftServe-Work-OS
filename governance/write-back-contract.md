...
| Surface | Only writer |
|---|---|
| `decisions/CLAUDE.md` "Recent Decisions" list — **the global one-line register**. Line shape: `- YYYY-MM-DD — [Title](file.md) — {area} · affects: {areas, or "—"} — one-line outcome`. The `{area}` and `affects:` fields are what make it a register rather than a list: any past decision is two reads (scan the register by module, then open the one file for the reasoning), and a cross-module call is findable from every module it touches, not only the one that made it. Without them the register is unfilterable and degrades into a chronological wall the moment it outgrows one screen | `/decision-log-entry` (and `/decision-doc` via the same rule) — append-only |
...
| `os-console/console.html` (the light-mode snapshot — derived output, never edited) | one builder (`os-console/build-console.js`, always from a COMMITTED tree), three callers: the turn-end hook `.claude/hooks/auto-commit.sh` (with the work it describes — a `console: rebuild snapshot` commit in the same push, or one more commit inside the pr-mode drain), the OS Console Rebuild button (`lib/snapshot.js`), a person by hand (`--ref HEAD`); the GitHub workflow is a manual fallback only. Conflicts resolve by regeneration (`.gitattributes` `merge=snapshot`), never by hand-merging; skills never touch it |