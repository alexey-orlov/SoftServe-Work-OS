...
| Documentation | `Documentation/` | The customer-facing documentation a team is sent — one self-contained site (`work-os-docs.html`, GitHub / Azure Repos switch) + one Word edition per platform, built from the single source in `src/content.js`. Gated. Change it with `/docs-update` (edit a specific thing, or `sync` to bring every stated fact back in line with the repo) |
