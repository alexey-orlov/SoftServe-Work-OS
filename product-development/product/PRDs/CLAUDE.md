...
- `{area}/reviews/` — Review artifacts for that area's PRDs: `[slug]-challenge-[YYYY-MM-DD].md` (`/prd-challenge` — dated, one per run), `[doc-slug]-red-team.md` (`/red-team`), `[feature-slug]-assumption-map.md` (`/assumption-map`), `[feature-slug]-premortem.md` (`/pre-mortem`)
