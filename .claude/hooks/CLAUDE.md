...
- [auto-commit.sh](auto-commit.sh) — Stop hook: commits the turn's work, and optionally merges it into the target branch, per the `settings:` block of `product-development/_meta/write-policy.yaml`. Ships disabled — a no-op until someone turns it on
