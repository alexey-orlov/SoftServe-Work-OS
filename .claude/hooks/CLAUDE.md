...
- [auto-commit.sh](auto-commit.sh) — Stop hook, the auto-sync engine: auto-commits the turn's work and pushes, per the `settings:` block of `governance/write-policy.yaml`. Direct strategies merge side branches into the target and hold gated paths back; the `pr` strategy (pull-request-only target) works on a branch per checkout, drains everyday commits to the target through self-merging pull requests (gh / az), and keeps gated commits on the branch for `/propose`. Also keeps the console's light-mode snapshot (`os-console/console.html`) current wherever `node` is on PATH — rebuilt from the committed tree and landed with the work it describes (a `console: rebuild snapshot` commit in the same push, or inside the pr-mode drain); direct mode fetches and rebases onto the target before pushing. Off until `/auto-sync on`
