...
```
/ralph-wiggum                          → Review the most recent PRD in product-development/product/PRDs/{area}/
/ralph-wiggum [paste document]         → Review pasted content
/ralph-wiggum product-development/product/PRDs/{area}/my-prd.md   → Review a specific file
```
...
**Time:** 3-5 minutes. No prep needed. Just point it at a document.
...
### Scope Boundary: Ralph vs /red-team

**Use /ralph-wiggum when:**
- You want the whole document reviewed — logic, evidence, scope, metrics, contradictions with repo context
- The doc is about to be shared and you want the embarrassing gaps caught first, memorably
- Output: Severity-bucketed review (CRITICAL/IMPORTANT/MINOR) with humor and repo cross-references

**Use /red-team when:**
- The bet itself needs attacking, not the document's craft — steelman-first, no persona
- You're about to commit resources and want kill criteria plus this-week tests
- Output: 3-5 kill-assumption contracts (Fails if / Evidence to get this week / Kill criterion / Cheapest test)

**Handoff:** If dimension 3 (Assumptions & Risks) dominates the review, suggest: "The core problem here is the bet, not the writing -- want /red-team to run the full attack?"
...
3. **If they said nothing:** Check `product-development/product/PRDs/{area}/` for the most recently modified file, then ask:
   ```
   I found [filename] in product-development/product/PRDs/{area}/ (last modified [date]). Want me to roast -- er, review -- this one?
...
- If this dimension dominates the review, say so and point to `/red-team` (attack the claims) or `/assumption-map` (build the full inventory)
...
Save to: `product-development/product/PRDs/{area}/reviews/[document-name]-ralph-review.md`
...
- Ralph complements `.claude/agents/reviewers/engineer-reviewer.md`, `.claude/agents/reviewers/designer-reviewer.md`, and `.claude/agents/reviewers/executive-reviewer.md`.
...
- `/red-team` - Full steelman-first attack when the bet (not the writing) is the problem
- `/assumption-map` - Build the ranked assumption inventory Ralph said was missing
