...
|--------|---------------|--------------|-----------------|
| Strategy Docs | `product-development/product/strategy/*.md` | feature name from chat | Strategic pillar alignment |
...
Before we draft, the skill checks what context exists...

**Checking:**
- `product-development/product/PRDs/{area}/` for any related feature PRDs
...
8. Who are the key stakeholders? (@ mention them if you have profiles in `product-development/product/strategy/business-context/stakeholders.md` or similar)
...
> More than 3-4 assumptions, or confidence ratings that are guesses? Run `/assumption-map` and import its Test First rows here — this table is the summary, not the method.
...
```
First draft created! Saved to `product-development/product/PRDs/{area}/[feature-name]-[stage].md`.
...
1. **Invoke each sub-agent explicitly:**
   ...
   Reading from `.claude/agents/reviewers/[agent-name].md`...
   ```
...
After stakeholder meetings, user research, or team discussions, provide what happened. The skill will help keep the PRD updated with:
- New insights
...
**Use dictation:**
Talking is faster than typing. Just ramble about the feature and the skill will structure it.
...
**PRD documents:**
- Active work: `product-development/product/PRDs/{area}/[feature-name]-[stage].md`
- When finalized: Move to `product-development/product/PRDs/{area}/[feature-name].md` for historical reference
- Evolve through stages: Team Kickoff → Planning Review → XFN Kickoff → Solution Review → Launch → Impact

### Register the feature and the initiative (prd-draft is the index producer)
...
- `product-development/product/strategy/` - Strategic pillar and alignment context
...
- [ ] **Saved to correct location:** `product-development/product/PRDs/{area}/` (NOT `product-development/product/PRDs/examples/` — that folder holds reference examples only)
- [ ] **Registered:** folder CLAUDE.md row appended; feature-index addition proposed (and applied only after PM confirmation); initiative page created or updated
...
- [ ] **Strategic fit references actual strategy:** Cites specific goals from `product-development/product/strategy/`, not generic strategy language
...
- [ ] **User quotes included:** If user research exists in `product-development/product/customers/`, at least one real quote is referenced
