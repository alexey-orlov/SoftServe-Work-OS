...
48 skills, invoked as `/{skill-name}`. Grouped below by use-case block; each skill carries its group in the `group:` frontmatter key.
...
| [`definition`](#definition) | Strategy, PRDs, metric definitions, impact sizing, experiment design, and the critique passes that stress-test them. | 14 |
...
- [create-tickets/](create-tickets/) — Break an agreed PRD into dev-backlog tickets, or push feature-request records as intake tickets — tracker MCP or text fallback
...
- [customize-os/](customize-os/) — Interactive instance customization: derive customized context files from the org's real artifacts (v1: house PRD template from 2–4 example briefs — blanked skeleton + guidance layer + fidelity validation); installs gated, in customer instances only
