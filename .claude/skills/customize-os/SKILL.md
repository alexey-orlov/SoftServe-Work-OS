...
| 9 | `instance-handoff` | `targets/instance-handoff.md` | **The gate before the instance leaves your hands** — mechanical lint clean, `.claude/` + `.github/` present and tracked, status file matches the tree, every command the repo names resolves. Pass/fail, not guidance |
...
- [ ] **State reconciled against the tree, not memory:** every sequence row re-checked
      against what the repo actually contains — a row saying `not started` whose effects are
      present (work done outside the skill), or `complete` whose artifacts are absent, is
      corrected in this run and the log says who did it and how. The status file is the
      resume point; a wrong one costs the next run more than an empty one
