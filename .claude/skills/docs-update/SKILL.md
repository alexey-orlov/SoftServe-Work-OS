...
(`work-os-docs.html`), one Word edition per platform, and the single source they are built
...
**Never edit the built files.** `work-os-docs.html` and the `.docx` files are outputs of
...
- **Platform switch:** nothing may show both platforms at once. Use `{gh:…|az:…}` inline or `platform("github", …)` / `platform("azure", …)` blocks; the `check` callout label is already switched. Word editions are per platform.
...
- **Illustrations:** slide screenshots are cropped to remove customer names; the SoftServe wordmark comes from `logo-inner.svg.txt` / `softserve-logo.png` — never a text substitute.
...
3. **Rebuild:** `cd Documentation/src && [ -d node_modules ] || npm install --no-audit --no-fund; node build.js`. The build prints the three outputs, `⚠ unresolved link …` for internal links that don't resolve and `⚠ both platforms …` where a sentence names GitHub and Azure together outside a token — fix both before reporting.
4. **Verify what can be verified:** `node -e "require('./content.js')"` parses; the article count printed by the build is what you expect; if headless Chrome is available (`"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu --enable-logging=stderr --v=0 --dump-dom file://…/work-os-docs.html`), no console errors; the `.docx` post-processing ran (build says so). Open nothing in the user's browser unasked.
