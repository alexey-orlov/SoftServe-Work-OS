...
| Competitor intel | competitor named with a new fact | `competitive-research/competitors/{slug}/teardown.md` (first intel: create the folder + CLAUDE.md stub, copy `processes/templates/competitor-teardown-template.md`) + refresh the affected `competitive-matrix.md` cells (and the `competitive-landscape.md` line when positioning-level) |
