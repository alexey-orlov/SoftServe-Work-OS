...
**What you get:** Structured notes with decisions, action items (with owners and dates), key insights, open questions, and next steps. Saved to `product-development/product/meetings/{type}/summaries/[date]-[topic].md`.
...
**For very long transcripts:** "This is a long transcript. The skill can process it all, but the output will be more accurate if we focus. Which approach do you prefer?
1. **Full processing** - The skill will cover everything but may compress less important sections
2. **Key segments only** - Provide which agenda items or topics matter most
3. **Chunked processing** - Paste the transcript in 2-3 parts and the skill will process each"
...
|--------|---------------|--------------|-----------------|
| Related PRDs | `product-development/product/PRDs/{area}/*.md` | feature from meeting | PRD context, decisions already made |
| Stakeholder Profiles | Stakeholder templates | attendees from meeting | Communication style, preferences, concerns |
| Strategy | `product-development/product/strategy/*.md` | feature or decision from meeting | Strategic alignment context |
| Previous Meetings | `product-development/product/meetings/*.md` | topic from this meeting | Historical context, related decisions |
...
Before processing, the skill checks if there's relevant context...

**Checking:**
- `product-development/product/meetings/` for any previous meetings on this topic
- Stakeholder profiles for communication style of attendees
- `product-development/product/PRDs/{area}/` for related feature context
...
As you share, the skill automatically extracts:
...
Once your content is provided, the skill responds:
...
(I'll use your preferred writing style from product-development/product/processes/writing-guides/*.md)
...
The skill automatically adjusts the format based on meeting type:
...
The skill matches your preferred style from `product-development/product/processes/writing-guides/*.md`:
...
If you don't specify who owns an action item, the skill will:
- Look at stakeholder info in `product-development/product/strategy/business-context/stakeholders.md` (or any profiles you add)
...
- Link to related PRDs in `product-development/product/PRDs/{area}/`
- Reference relevant strategy docs in `product-development/`
...
- PRDs in `product-development/product/PRDs/{area}/*.md` and `product-development/product/PRDs/{area}/*.md` (launch dates, milestone dates)
- Previous meeting notes in `product-development/product/meetings/*.md` (prior commitments, deadlines)
- Strategy docs in `product-development/product/strategy/*.md` (quarter end dates, OKR deadlines)
...
After processing, You'll be offered:
...
For customer interviews, the skill will add:
...
If sensitive topics are detected:
...
The skill will structure it into proper notes automatically.
...
- Any relevant background the skill should know

This helps the skill extract the RIGHT insights, not just ANY insights.

### 4. Correct and Refine

After the skill creates the notes, review and edit:
- Add context that may have been missed
...
- `product-development/product/meetings/{type}/summaries/[date]-[topic].md`

Over time, the skill will spot patterns:
...
**Pulls from:**
- `product-development/product/PRDs/{area}/` - Feature context for meeting
- Stakeholder profiles - Communication preferences and concerns
- `product-development/product/strategy/` - Strategic alignment context
- `product-development/product/meetings/` - Previous decisions and context
