...
- [claude-admin-console.html](claude-admin-console.html) — The Claude Code page of the Claude admin console (organization switches, Managed settings box) for *Require approval for every Claude Code write*; layout and names follow Anthropic's documentation.
- [claude-code-mode-selector.html](claude-code-mode-selector.html) — The Claude Code desktop app under the organization rule: the mode selector without Auto and Bypass permissions, and the approval card before a file change — for *Require approval for every Claude Code write*.
