  ...
  sections: [
    ...
    { id: "overview", title: "Overview", articles: [
      ...
      { id: "work-os-101", title: "Work OS 101", audience: "Everyone", time: "8 min read", blocks: [
        ...
        callout("tip", "New here? Read [Skills and agents](#/overview/skills) to see what Claude can do for you, then [Context system](#/overview/context) to see what it knows. [From idea to handoff](#/overview/workflow) shows those skills working as one chain, and [Everyday work](#/overview/everyday) covers the five things you do with the Work OS week to week. When you're ready to roll it out, start with [Setup › Overview](#/setup/overview)."),
      ]},
      ...
      { id: "context", title: "Context system", audience: "Everyone", time: "6 min read", blocks: [
        ...
        table(["Item", "Type", "What's in it", "How it's used"], [
          ...
          ["`prototypes/`", "Deliverable", "Clickable HTML prototypes with per-prototype feedback logs and challenge reports; `design-system/` caches the Figma tokens they are built against", "`/prototype` builds, `/prototype-challenge` critiques, `/prototype-feedback` applies review rounds; linked from job specs and handoffs"],
      ...
      { id: "workflow", title: "From idea to handoff", audience: "Everyone", time: "5 min read", blocks: [
        lead("[Skills and agents](#/overview/skills) lists what each skill does on its own. This article is the order they run in. One feature travels four stages — idea, discovery, definition, handoff — and each skill reads what the one before it wrote: the evidence gathered in discovery is the evidence the job spec cites, so nothing is re-typed and nothing is re-decided."),
        h2("The four stages", "stages"),
        table(["Stage", "What happens", "Skills"], [
          ["**Idea**", "One page states the bet: the problem, who has it, why now. The first draft marks what it does not know instead of inventing it.", "`/prd-draft`"],
          ["**Discovery**", "The gaps that need evidence get filled — customers, competitors, the numbers behind the case. Parts of this only a person can do: Claude prepares the interview and works up what comes back.", "`/interview-guide` · `/user-research-synthesis` · `/competitor-analysis` · `/impact-sizing` · `/feature-metrics` · `/code-qa`"],
          ["**Definition**", "The bet is challenged from every side, cut into independently shippable jobs, and each job gets a contract precise enough to build from — and, where it earns its place, something clickable built from that contract.", "`/prd-challenge` · `/jobs-breakdown` · `/job-spec-draft` · `/job-spec-challenge` · `/prototype`"],
          ["**Handoff**", "One gate decides whether the requirements can leave you. Past it, the same package becomes dev-backlog tickets.", "`/pm-handoff` · `/create-tickets`"],
        ], [1300, 5100, 3200]),
        ...
        callout("why", "A gap you can see is worth more than a sentence that reads well. `[GAP:]` markers are what [the handoff gate](#/overview/workflow/gate) counts at the end, so an unclosed gap blocks the handoff instead of reaching the BA unnoticed."),
        ...
        p("`/prd-challenge` and `/job-spec-challenge` are the passes that stop a comfortable draft from going forward. Both run every critique lens at once and blind to each other — the [reviewer agents](#/overview/skills/agents), an assumption map, a red team, a rehearsal of the failed launch once there is a solution and a rollout to test — then merge everything into one report, led by the assumptions nobody has verified and the next research step for each."),
        bullets([
        ...
        callout("tip", "Work that is not about one feature — filing a call, catching up on the week, teaching the Work OS something it got wrong — is in [Everyday work](#/overview/everyday)."),
      ]},

      { id: "everyday", title: "Everyday work", audience: "Everyone", time: "6 min read", blocks: [
        lead("Five things you do with the Work OS whatever feature you are on: bring new material in, find out what it already knows, work with the rest of the team, teach it what it got wrong, and keep it healthy. Each one is a sentence you say to Claude — the skill names below are shortcuts, and where no skill exists you simply ask."),
        h2("Bringing new material in", "material"),
        p("Everything the team learns arrives as something: a call recording, a deck, a thread, a customer's e-mail. Material that stays in your inbox is material Claude cannot use."),
        ...
        p("Sensitive material is handled by rule, not by memory: customer-side people are named by role only, and salary, revenue and health details are kept out of the summary and the filed transcript alike. The Privacy Contract in `CLAUDE.md` lists what may never be committed at all — 1:1 and coaching notes, performance feedback, comp and board material, credentials. You can always check what actually landed:"),
        say("Scan last week's interview summaries for personal names, salary or health details — and tell me the rules you applied."),
        h2("Finding out what it already knows", "know"),
        ...
        callout("tip", "The gap in the answer is the useful part. Add the missing context — or write it down as homework — before you run the skill that needed it. [What one skill run looks like](#/overview/workflow/run) shows how to ask a skill what it will read."),
        h2("Working with the rest of the team", "team"),
        table(["What you want", "What to use"], [
          ...
          ["To get your draft in front of the team today", "Nothing, if auto-sync is on — see [Everyday changes](#/overview/changes/everyday). Share the file's path in the repository rather than a copy: a copy goes stale the moment you edit again."],
          ["To see what a teammate changed — and undo it if it was wrong", "Ask. Every change is saved as its own version, so nothing is lost: see [Undoing a change](#/overview/changes/undo)."],
        ], [3400, 6200]),
        h2("Teaching it what it got wrong", "improve"),
        p("This is the part that makes the Work OS worth keeping. A correction you make in a chat is gone tomorrow; a correction you file becomes how the team works."),
        ...
        callout("note", "Everything in this section touches a gated file, so none of it lands silently: you approve the write, then a Work OS admin approves it reaching the team. [Changes to gated files](#/overview/changes/gated) walks through what you will see."),
        h2("Keeping the Work OS healthy", "healthy"),
        p("Mostly the Work OS admin's part — one or two champions from the product team who work in the Work OS every day, not an IT function. It is not a developer job either: Claude does the saving and syncing, while the admin reads proposals, decides, and keeps the steering files honest."),
        ...
        table(["What you want", "What to use"], [
          ...
          ["To protect one more file", "*\"Add the competitive landscape page to the gated list.\"* See [Changing the gated list](#/overview/changes/list)."],
        ], [3400, 6200]),
      ...
      { id: "changes", title: "How changes reach the team", audience: "Everyone", time: "4 min read", blocks: [
        ...
        p("None of this is about reading: everyone reads everything in the Work OS. What the roles differ on is what reaches the shared repository without someone else looking at it first."),
        ...
        platform("azure", p("The list the repository itself enforces is refreshed once by hand afterwards: the pull request description carries the exact line and where to paste it (**Project settings › Repositories › Policies › Branch policies › Automatically included reviewers › Path filter**). It is the same field as [Set up the repository, step 4](#/setup/repository/s4).")),
      ]},
    ]},
